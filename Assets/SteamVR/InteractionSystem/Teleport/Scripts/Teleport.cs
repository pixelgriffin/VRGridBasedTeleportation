...
namespace Valve.VR.InteractionSystem
{
	...
	public class Teleport : MonoBehaviour
    {
        ...
        {
            return InteractionSystem.Player.instance;
        }
		...
		private void UpdatePointer()
		{
            ...
            Debug.Log("pointedAt: " + hitInfo.point);
            telePosition = hitInfo.point;
		...
		private void TeleportPlayer()
		{
			...
			//if ( teleportArea != null )
			{
				//if ( floorFixupMaximumTraceDistance > 0.0f )
				{
					...
					if ( Physics.Raycast( teleportPosition + 0.05f * Vector3.down, Vector3.down, out raycastHit, floorFixupMaximumTraceDistance, floorFixupTraceLayerMask ) )
					{
                        ...
                        Debug.Log("eleport pos: " + teleportPosition);
					}
				}
			}
            ...
            Debug.Log("pointed at: " + telePosition);

            Vector3 playerFeetOffset = player.trackingOriginTransform.position - player.feetPositionGuess;
            player.trackingOriginTransform.position = telePosition + playerFeetOffset;
        }
		...
		//-------------------------------------------------
		public bool IsTeleportButtonDown( Hand hand )
		{
			if ( IsEligibleForTeleport( hand ) )
			{
				if ( hand.noSteamVRFallbackCamera != null )
