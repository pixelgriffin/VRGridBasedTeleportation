...
{
    public Hand controllerReference;
...
public class VRUIPointer : MonoBehaviour
{
    ...
    protected Hand controllerReference
    {
        get
        {
            return controller;
    ...
    protected VRInputModule cachedVRInputModule;
    ...
    public virtual VRInputModule SetEventSystem(EventSystem eventSystem)
    {
        if (!eventSystem)
        {
        ...
        if (!(eventSystem is VREventSystem))
        {
            eventSystem = eventSystem.gameObject.AddComponent<VREventSystem>();
        }

        return eventSystem.GetComponent<VRInputModule>();
    }
    ...
    {
        var vrtkEventSystem = FindObjectOfType<VREventSystem>();
    ...
    public virtual bool PointerActive()
    {
        ...
        else
        {
            ...
            lastPointerPressState = (controller != null ? true : false);
    ...
    {
        return (controller != null ? controller.grabGripAction.GetState(controller.handType) : false);
    }
    ...
    {
        pointerOriginTransform = originalPointerOriginTransform;//(originalPointerOriginTransform == null ? VRTK_SDK_Bridge.GenerateControllerPointerOrigin(gameObject) : originalPointerOriginTransform);
    ...
    protected virtual void LateUpdate()
    {
        if (controller != null)
        {
            pointerEventData.pointerId = (int)controller.GetDeviceIndex();//(int)VRTK_ControllerReference.GetRealIndex(controllerReference);
        }
